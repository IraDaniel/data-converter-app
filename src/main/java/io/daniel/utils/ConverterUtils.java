...
public class ConverterUtils {
    ...
    public static List<OrderResponse> convert(List<ParsedLine> parsedLines, String fileName) {
        return parsedLines.stream().map(parsedLine -> {
            Order request = parsedLine.getOrder();
            if (parsedLine.isSuccess()) {
                return new OrderResponse(request.getOrderId(), request.getAmount(), request.getComment(), fileName, parsedLine.getLine(), parsedLine.getResult());
            } else {
                return new OrderResponse(null, null, null, fileName, parsedLine.getLine(), parsedLine.getResult());
            }
